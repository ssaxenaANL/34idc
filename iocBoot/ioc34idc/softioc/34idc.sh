...
# Manually set IOC_STARTUP_DIR if 34idc.sh will reside somewhere other than softioc
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/34idc/iocBoot/ioc34idc/softioc
...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select 34idc, but not 34idc.sh
...
start() {
    if checkpid; then
        ...
        cd ${IOC_STARTUP_DIR}
        # Run 34idc inside a screen session
...
run() {
    if checkpid; then
        ...
        ${ECHO} "Starting ${IOC_NAME}"
        cd ${IOC_STARTUP_DIR}
        # Run 34idc outside of a screen session, which is helpful for debugging
        ${IOC_CMD}
    fi
}

start_medm() {
    ${IOC_STARTUP_DIR}/../../start_MEDM_34idc
}

start_caqtdm() {
    ${IOC_STARTUP_DIR}/../../start_caQtDM_34idc
}
